...
func Generate(q, a, p string) error {
	...
	palette := []color.Color{
		...
		color.RGBA{0xBF, 0x62, 0x00, 0xFF},
		color.RGBA{0x80, 0x41, 0x00, 0xFF},
		color.RGBA{0x40, 0x21, 0x00, 0xFF},
		color.Black,
	}

	palettedImage1 := image.NewPaletted(askI.Image().Bounds(), palette)
	draw.FloydSteinberg.Draw(palettedImage1, askI.Image().Bounds(), askI.Image(), image.ZP)
	palettedImage2 := image.NewPaletted(answerI.Image().Bounds(), palette)
